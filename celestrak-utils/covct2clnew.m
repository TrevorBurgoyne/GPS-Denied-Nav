...
function [classcov,tm] = covct2clnew(cartcov, cartstate, anom )
    ...
    fprintf(1, ' %14.6f %8.6f %10.5f %10.5f %10.5f %10.5f %10.5f \n',...
            ax, eccx, inclx*rad, nodex*rad, argpx*rad, nux*rad, mx*rad );
        ...
        tm(3,6) =  p3*(hz*(ry*hx - rx*hy)/h_squared);  % vz*nx/h_squared; 
...
%       tm(6,6) = p9 * ( p8 * (h*rz + r_dot_v*(ry*hx - rx*hy)/h) - r_dot_v*h*(2*vz*magr^2 - 2*r_dot_v*rz) );
        ...
        if strcmp(anom,'meana') == 1 || strcmp(anom,'meann') == 1  % 1 is true
           ...
           % p6 = -sin(nu)*(sign((ecc*cos(nu) + 1)) + 1.0 - 2.0*ecc^2 - ecc^3*cos(nu)) / ( (ecc*cos(nu) + 1.0)^2 * sqrt(1-ecc^2) )  % dm/de
            tm(6,1) = tm(6,1)*dMdnu + tm(2,1)*dMde;
            tm(6,2) = tm(6,2)*dMdnu + tm(2,2)*dMde;
            tm(6,3) = tm(6,3)*dMdnu + tm(2,3)*dMde;
            tm(6,4) = tm(6,4)*dMdnu + tm(2,4)*dMde;
            tm(6,5) = tm(6,5)*dMdnu + tm(2,5)*dMde;
            tm(6,6) = tm(6,6)*dMdnu + tm(2,6)*dMde;
        end 
