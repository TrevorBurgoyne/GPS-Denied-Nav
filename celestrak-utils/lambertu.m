...
%dtsec = dtsec / tusec;
        ...
        if (nrev == 0)
            ...
            if (de == 'd')  % dm == 's' seemed better other way?????????????
            ...
            [c2,c3] = findc2c3( psiaux );
            y= magr1 + magr2 - ( vara*(1.0-psiaux*c3) / sqrt(c2) );
            taux    = (x^3*c3 + vara*sqrt(y)) * oomu;
...
%             end;    
        ...
        if (strcmp(errorl, '      ok') ~= 0)
            ...
            fprintf(outfile,'%10s %3i %3i %2s %2s %11.7f %11.7f %11.7f %11.7f %11.7f %11.7f %11.7f %11.7f %11.7f case %11.7f %11.7f %11.7f %11.7f %11.7f ', ...
                     errorl, loops, nrev, dm, de, dtwait, dtnew, y, x, v1dv(1), v1dv(2), v1dv(3), v2dv(1), v2dv(2), v2dv(3), lower, upper, psinew, dtdpsi, ecc); %(dtnew - dtsec)/dtdpsi, ecc );  % c2dot, c3dot
            fprintf(1,'C%3i %3i %2s %2s %11.7f %11.7f %11.7f %11.7f %11.7f %11.7f  %11.7f dnu %11.7f \n', ...
                   loops, nrev, dm, de, dtnew, magr1, magr2, vara, y, x, psinew, acos(cosdeltanu)*180.0/pi)
        else
            fprintf(outfile,'#%s \n',errorl);
            fprintf(1,'#%s \n',errorl);
        end
