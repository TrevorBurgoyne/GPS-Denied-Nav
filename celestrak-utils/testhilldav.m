...
% old end of loop...
 ...
 %   sound(y,Fs)
     ...
     rad = 180.0 / pi;    
     rtgteci = [0.000000 9380.502297  5093.197188]';   
     vtgteci = [-6.110899   3.222217   1.749521]';
     ...
     hrokm = [79.378750  0.00  -93.843398]';
     hvokm = [-0.038118  -0.068157  -0.031665]';
     ...
     rhrsw = transmat' * hrokm;
     vhrsw = transmat' * hvokm;
     
     % form int vector
     rinteci = rhrsw + rtgteci;
     vinteci = vhrsw + vtgteci;
         
     fprintf(1,'rinteci RSW only %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n', ...
              rinteci(1), rinteci(2), rinteci(3), vinteci(1), vinteci(2), vinteci(3) );
     
          
          
     % now try EQCm conversion
     [rinteci, vinteci]   = EQCM_to_ECI_RTN_sal( rtgteci, vtgteci, hrokm, hvokm );     % find int pos eqcm

     fprintf(1,'rinteci EQCM hold pos %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n', ...
              rinteci(1), rinteci(2), rinteci(3), vinteci(1), vinteci(2), vinteci(3) );

     %stk ans
     rintans = [0.000000000001      9495.039900814565     5048.602269265176]';
     vintans = [-6.088186588973     3.203827617391     1.703505361584]';
     fprintf(1,'STK answer            %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n', ...
              rintans(1), rintans(2), rintans(3), vintans(1), vintans(2), vintans(3) );
     
     rdiff = rinteci - rintans;
     fprintf(1,'diff from STK answer  %20.13f  %20.13f  %20.13f \n', rdiff (1), rdiff (2), rdiff (3) );
     ...
     plotorb(rtgteci, vtgteci, 16678.1385, 'red')      
      ...
      [rinteciC, vinteciC] = EQCM_to_ECI_RTN_sal( r, v, hrokm, hvokm );     % find int pos eqcm
      fprintf(1,'rinteci EQ rtn cir %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n', ...
      ...
      fprintf(1,'rinthill EQCM     %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n', ...
      ...
      fprintf(1,'rinthill EQCM cir %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n', ...
              rinthill(1), rinthill(2), rinthill(3), vinthill(1), vinthill(2), vinthill(3) );
      
      % try NTW frame - hold velocity direction constant
      % not sure this one is correct
      [rinteciz, vinteciz]   = EQCM_to_ECI_NTW_sal( rtgteci, vtgteci, hrokm, hvokm );     % find int pos eqcm
      fprintf(1,'rinteci EQCM ntw     %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n', ...
     ...
     no_kozai = 15.59114070;                   % revs per day
     ecco = 0.000577;
     inclo = 51.6190/rad;
     ...
     a1    = (no_kozai*tumin)^(-2.0/3.0);     % er
     
     % initl (inside init)
     ak    = (xke / no_kozai)^x2o3;          % er
     d1    = 0.75 * j2 * (3.0 * cosio2 - 1.0) / (rteosq * omeosq);
     del1   = d1 / (ak * ak);
     adel  = ak * (1.0 - del1 * del1 - del1 *...
         (1.0 / 3.0 + 134.0 * del1 * del1 / 81.0));
     del   = d1/(adel * adel);
     no_unkozai  = no_kozai / (1.0 + del);   % rad/min 

     ao    = (xke / no_unkozai)^x2o3;        % er
    
     fprintf(1,'nokoz  %11.6f   unkoz  %11.6f  a  %11.6f er a1  %11.6f er \n', no_kozai, no_unkozai, ao, a1);
     fprintf(1,'nokoz  %11.6f   unkoz  %11.6f  a  %11.6f km a1  %11.6f km \n', no_kozai, no_unkozai, ao*re, a1*re);
     
     % now go in reverse
     
     % assume for 1st guess that no_kozai = no_unkozai
     ak    = (xke / no_unkozai)^x2o3;          % er
    
     d1    = 0.75 * j2 * (3.0 * cosio2 - 1.0) / (rteosq * omeosq);  % same both ways
     del1   = d1 / (ak * ak);
     adel  = ak * (1.0 - del1 * del1 - del1 *...
         (1.0 / 3.0 + 134.0 * del1 * del1 / 81.0));
     del   = d1/(adel * adel);
   
     no_kozai = no_unkozai * (1.0 + del);

     fprintf(1,'nokoz  %11.6f   unkoz  %11.6f  a  %11.6f km a1  %11.6f km \n', no_kozai*1440/(2*pi), no_unkozai*1440/(2*pi), ao*re, a1*re);
