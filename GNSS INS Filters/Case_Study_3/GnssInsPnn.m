function P_NN = GnssInsPnn( satAzimuths, satElevations, Rcode, Rphase, Pins, wavelength )
%
% P_NN = GnssInsPNN( satAzimuths, satElevations, Rcode, Rphase, Pins, wavelength )
%
% SUMMARY: 
%    GnssInsPNN computes the covariance matrix of the carrier phase ambiguities using a combination
%    of GNSS pseudorange and carrier phase GPS data, and an initial INS position solution.  The
%    results is based on least-squares positioning, and is therefore indicative of performance
%    following a complete GNSS data outage.
%
% INPUT:
...
%  - Pins: Covariance matrix of the INS-derived position solution (North, East, Vertical) in units
%          of metres^2.
...
%     - Use the given covariance matrices, the design matrix from the above step and the INS
%       position accuracy to compute the least-squares covariance matrix of the entire state vector.  
%       From this matrix the sub-matrix corresponding to the ambiguity states is then extracted an 
%       returned.
%
% COPYRIGHT:
%    (c) 2009
...
if size( Pins, 1 ) ~= size( Pins, 2 ) || size( Pins, 1 ) ~= 3
   error( 'Covariance matrix of the INS-derived position (Pins) is not 3x3' );
end
...
Hins = [ eye(3) , zeros( 3, N-1 ) ];
H = [ Hcode ; Hphase ; Hins ];

Rcode = D * Rcode * D';
Rphase = D * Rphase * D';
R = blkdiag( Rcode, Rphase, Pins );
